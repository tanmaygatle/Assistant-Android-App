...
{
  if ((p->s = MY_REALLOC(p->s, n * p->adim * sizeof(spf_t))) != NULL)
    p->m = n;
...
unsigned long spf_buf_write(spfbuf_t *buf, FILE *f)
{
  ...
  while (nwritten < buf->n) {
    ...
    /***********************    
     for (jj=0; jj<buf->dim; jj+=500)
      for(kk=jj; kk<jj+4; kk++)
#ifdef __ANDROID_API__
      __android_log_print(ANDROID_LOG_DEBUG, "spf", "-spf_buf_write %f \n", *(p+kk));
#else
    printf("-spf_buf_write %f \n", *(p+kk));
    #endif 
    **************************/
				    ...
				    long cflag, float frate, const spfield_t *vh, size_t nbytes)
{
  ...
  /**************************
  printf("spf_output_stream_open  dim=%i, flag=%ld,  rate=%f \n", dim, flag, rate);
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "spf", "-spf_output_stream_open dim=%i, flag=%ld,  rate=%f \n", dim, flag, rate);
#endif
  ***************************/
