...
void spf_header_free(spfheader_t *p)
{
  ...
  if (p) {
	...
	if (p->field[i].name)
	  MY_FREE(p->field[i].name);
	if (p->field[i].value)
	  MY_FREE(p->field[i].value);
      }
      MY_FREE(p->field);
    }
    MY_FREE(p);
...
int spf_header_add(spfheader_t *p, const spfield_t *fld)
{
  ...
  if (n) {
    ...
    /* reallocate memory */
    if ((fp = (spfield_t *)MY_REALLOC(p->field, (in + n) * sizeof(spfield_t))) == NULL) {
  ...
  if (c == '<') { /* variable length header */
	...
	return(NULL);
      ...
      *p = 0;
      ...
      /* reallocate memory */
      if ((hp->field = (spfield_t *)MY_REALLOC(hp->field, n * sizeof(spfield_t))) == NULL) {
	fprintf(stderr, "spf_header_read(): cannot allocate memory\n");
	spf_header_free(hp);
	return(NULL);
      }
