...
int fft_init(unsigned long npts)
{
  ...
  if (npts) {
    ...
    /*********************************
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "fft ", "-fft_init, fftbuf npts=%lu\n", npts);
#endif
    *********************************/
    if ((_fftbuf = (float *)MY_REALLOC(_fftbuf, npts * sizeof(float))) == NULL) {
    ...
    /*********************************
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "fft ", "-fft_init, w1c n4=%d\n", n4);
#endif
    *********************************/
    if ((w1c = (float *)MY_REALLOC(w1c, n4 * sizeof(float))) == NULL) {
    ...
    /*********************************
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "fft ", "-fft_init, w3c  n4=%d\n", n4);
#endif
    *********************************/
    if ((w3c = (float *)MY_REALLOC(w3c, n4 * sizeof(float))) == NULL) {
      fprintf(stderr, "[SPro error %d] FFTInit(): cannot allocate FFT kernel", SPRO_ALLOC_ERR);
      MY_FREE(_fftbuf); MY_FREE(w1c);
      return(SPRO_ALLOC_ERR);
    }
        /*********************************
#ifdef __ANDROID_API__
  __android_log_print(ANDROID_LOG_DEBUG, "fft ", "-fft_init,  jx0 npts=%lu\n", npts);
#endif
	*********************************/
    if ((jx0 = (long *)MY_REALLOC(jx0, npts * sizeof(long) / 3)) == NULL) {
      fprintf(stderr, "[SPro error %d] FFTInit(): cannot allocate FFT kernel", SPRO_ALLOC_ERR);
      MY_FREE(_fftbuf); MY_FREE(w1c); MY_FREE(w3c);
      return(SPRO_ALLOC_ERR);
    }
...
int dct_init(unsigned short nin, unsigned short nout)
{
  ...
  if (nin && nout) {
    /*********************************
#ifdef __ANDROID_API__
      __android_log_print(ANDROID_LOG_DEBUG, "fft ", "-dct_init,  nin=%d nout=%d\n", nin, nout);
#endif
    *********************************/

    _dctk = (float **)MY_REALLOC(_dctk, nout * sizeof(float *));
    if (_dctk == NULL) {
    ...
    /*********************************
#ifdef __ANDROID_API__
	__android_log_print(ANDROID_LOG_DEBUG, "fft ", "-dct_init 2, i=%d,  nin=%d nout=%d\n", i, nin, nout);
#endif
    *********************************/

      if ((_dctk[i] = (float *)MY_REALLOC(_dctk[i], nin * sizeof(float))) == NULL) {
